pcall(function()
    ...
    local oldGui = pg and pg:FindFirstChild("DraconicHubGui")
    if oldGui then
        oldGui:Destroy()
    end
end)
...
local OPT_INTERVALS = {
    TRACER = 1 / 40,
    NEXTBOT_KEEPALIVE = 0.2,
    PLAYER_ESP_MONITOR = 0.35,
    TICKET_SCAN = 0.1,
    TIMER_VISIBILITY = 0.2,
    AUTO_WIN = 0.1,
    AUTO_MONEY_CHECK = 0.4,
    AUTO_TICKET = 0.05,
    ANTI_NEXTBOT = 0.1,
    AUTO_CARRY = 0.12,
    CHARACTER_REF = 0.25,
...
local function connectThrottled(signal, interval, callback)
    local elapsed = 0
...
local function getSceneNode(key, force)
    local now = os.clock()
    if force or (now - SceneCache.ts) >= SceneCache.interval then
        local gameFolder = workspace:FindFirstChild("Game")
        SceneCache.gameFolder = gameFolder
        SceneCache.gamePlayers = gameFolder and gameFolder:FindFirstChild("Players") or nil
        SceneCache.gameEffects = gameFolder and gameFolder:FindFirstChild("Effects") or nil
        SceneCache.gameMap = gameFolder and gameFolder:FindFirstChild("Map") or nil
        SceneCache.gameMapParts = SceneCache.gameMap and SceneCache.gameMap:FindFirstChild("Parts") or nil
        SceneCache.tickets = SceneCache.gameEffects and SceneCache.gameEffects:FindFirstChild("Tickets") or nil
        SceneCache.ragdolls = gameFolder and gameFolder:FindFirstChild("Ragdolls") or nil
        SceneCache.spawns = SceneCache.gameMapParts and SceneCache.gameMapParts:FindFirstChild("Spawns") or nil
        SceneCache.npcs = workspace:FindFirstChild("NPCs")
        SceneCache.securityPart = workspace:FindFirstChild("SecurityPart")
        SceneCache.ts = now
    end
...
local PLAYER_ESP_URL = "https://raw.githubusercontent.com/Ksjsowos/29401/refs/heads/main/Online%20Script/Esp.lua"
local NEXTBOT_ESP_URL = "https://raw.githubusercontent.com/Ksjsowos/29401/refs/heads/main/Online%20Script/NextbotESP.lua"
...
local function scanForTickets()
    ...
    if ExternalTicketESPLoaded and _G.UpdateTicketESP then
        ...
        local gameFolder = getSceneNode("gameFolder")
        if gameFolder then
...
function updateBotTracers()
    ...
    local playersFolder = getSceneNode("gamePlayers")
    ...
    local npcsFolder = getSceneNode("npcs")
    if npcsFolder then
...
function startPlayerTracers()
    ...
    playerTracerConnection = connectThrottled(RunService.RenderStepped, OPT_INTERVALS.TRACER, updatePlayerTracers)
end
...
function startBotTracers()
    ...
    botTracerConnection = connectThrottled(RunService.RenderStepped, OPT_INTERVALS.TRACER, updateBotTracers)
end
    ...
    if value then
        ...
        if not ExternalNextbotESPLoaded then
            local success, errorMsg = pcall(function()
                ExternalNextbotESP = loadstring(game:HttpGet(NEXTBOT_ESP_URL))()
        ...
        if not nextbotLoop then
            nextbotLoop = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.NEXTBOT_KEEPALIVE, function()
...
PlayerToggle:OnChanged(function(value)
    if value then
        ...
        if not ExternalESPLoaded then
            local success, errorMsg = pcall(function()
                ExternalESP = loadstring(game:HttpGet(PLAYER_ESP_URL, true))()
        ...
        if not playerLoop then
            playerLoop = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.PLAYER_ESP_MONITOR, function()
        ...
        if ExternalESP and ExternalESPLoaded then
            ...
            elseif type(ExternalESP) == "function" then
                pcall(ExternalESP)
            end
        end
...
TicketToggle:OnChanged(function(value)
    if value then
        ...
        if not ticketLoop then
            ticketLoop = connectThrottled(RunService.RenderStepped, OPT_INTERVALS.TICKET_SCAN, function()
...
TimerDisplayToggle:OnChanged(function(state)
    if state then
        ...
        timerDisplayLoop = connectThrottled(RunService.RenderStepped, OPT_INTERVALS.TIMER_VISIBILITY, function()
...
function startAutoWin()
    ...
    AutoWinConnection = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.AUTO_WIN, function()
        local securityPart = getSceneNode("securityPart")
...
function initAutoReviveModule()
    ...
    local function isPlayerDowned(pl)
        ...
        local ragdollsFolder = getSceneNode("ragdolls")
...
function startAutoMoneyFarm()
    ...
    AutoMoneyFarmConnection = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.AUTO_MONEY_CHECK, function()
...
AutoTicketFarmToggle:OnChanged(function(state)
    ...
    if state then
        ...
        AutoTicketFarmConnection = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.AUTO_TICKET, function()
            local character = LocalPlayer.Character
            if not character then return end
            ...
            local tickets = getSceneNode("tickets")
...
function handleAntiNextbot()
    ...
    playersFolder = getSceneNode("gamePlayers")
    if playersFolder then
        ...
        if nextbotHrp then
            ...
            if distance <= featureStates.AntiNextbotDistance then
                ...
                elseif featureStates.AntiNextbotTeleportType == "Spawn" then
                    spawnsFolder = getSceneNode("spawns")
...
AntiNextbotToggle:OnChanged(function(state)
    ...
    if state then
        antiNextbotConnection = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.ANTI_NEXTBOT, function()
            if not featureStates.AntiNextbot then return end
            ...
            if isTooClose then
                safePart = getSceneNode("securityPart")
                if safePart then
...
local function startAutoCarry()
    ...
    AutoCarryConnection = connectThrottled(RunService.Heartbeat, OPT_INTERVALS.AUTO_CARRY, function()
...
connectThrottled(RunService.Heartbeat, OPT_INTERVALS.CHARACTER_REF, function()
...
VisualNametagDropdown = VisualsTab:AddDropdown("VisualNametagDropdown", {
    ...
    Default = "Ignore",
    Callback = function(Value)
        local gamePlayers = getSceneNode("gamePlayers")
        playerFolder = gamePlayers and gamePlayers:FindFirstChild(game.Players.LocalPlayer.Name)
...
connectThrottled(game:GetService("RunService").Heartbeat, OPT_INTERVALS.NAMETAG_SYNC, function()
    local gamePlayers = getSceneNode("gamePlayers")
    playerFolder = gamePlayers and gamePlayers:FindFirstChild(game.Players.LocalPlayer.Name)
    if playerFolder and Options.VisualNametagDropdown and Options.VisualNametagDropdown.Value ~= "Ignore" then
        if Options.VisualNametagDropdown.Value == "None" then
    ...
    if Options.PlayerToggle and Options.PlayerToggle.Value then
        if not ExternalESPLoaded or not _G.ExternalESPRunning then
            ...
            local success = pcall(function()
                ExternalESP = loadstring(game:HttpGet(PLAYER_ESP_URL))()
                ExternalESPLoaded = true
                _G.ExternalESPRunning = true
            end)
